...
#define QMC5883_CONR1 0x09
#define QMC5883_CONR2 0x0A
#define HMC58X3_X_SELF_TEST_GAUSS (+1.16)                       //!< X axis level when bias current is applied.
#define HMC58X3_Y_SELF_TEST_GAUSS (+1.16)   //!< Y axis level when bias current is applied.
#define HMC58X3_Z_SELF_TEST_GAUSS (+1.08)                       //!< Y axis level when bias current is applied.
#define SELF_TEST_LOW_LIMIT  (243.0/390.0)   //!< Low limit when gain is 5.
...
static void Mag_init() {
  ...
  */
  // leave test mode
  i2c_writeReg(MAG_ADDRESS ,QMC5883_CONR1 ,0x1D ); //Configuration Register 1  -- 11 00 00 01   64 over sample ratio, 2G full scale, 10Hz output data rate, continous mode
  i2c_writeReg(MAG_ADDRESS ,QMC5883_CONR2 ,0x00000000 ); //Configuration Register B  -- 00 00 00 00   pointer roll-over normal, soft reset normal, interupt disabled    
  i2c_writeReg(MAG_ADDRESS ,0x0B ,0x01 ); //Set point and reset period set to document recommendation
  delay(100);
}
